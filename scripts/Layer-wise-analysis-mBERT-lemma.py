from mbert import encode_data_and_get_lemma_embeddings, create_data_loader, clear_old_model, train_and_evaluate, eval_test_model
...
RESULTS_DIR = os.path.join(os.path.dirname(script_dir), 'training_results_LEMMA')
...
language_dataset_loaders = {} # Dictionary to store the data loaders for train, val, test set for every language
...
for language in languages:
    ...
    # Create noun embeddings and data loaders for full form of nouns
...
for source_language in languages:
    ...
    plot_dir = os.path.join(RESULTS_DIR, f'plots_{source_language}_mBERT_LEMMA')
    ...
    for layer in selected_layers:
        ...
        for seed_value in seed_values:
            ...
            for target_language in languages:
                ...
                plot_cm_per_seed(test_true_labels, test_predictions, layer, seed_value, target_language, plot_dir)
    ...
    result_filename = f"{source_language}_results_mBERT_layer_LEMMA.json"
    ...
    dataframes = [] # list to store DataFrames for each target language
    ...
    plt.tight_layout()
    # Save the plot
    plot_path = os.path.join(plot_dir, f"test_f1s_{source_language}_mBERT_layer_LEMMA.png")
